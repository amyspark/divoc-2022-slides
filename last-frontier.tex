...
Encoded value    & Function type                \\%
\midrule%
$\mathtt{0003h}$ & $\begin{cases}%
                Y = (aX+b)^{g} & X \geq d \\%
                Y = cX         & X < d    \\%
              \end{cases}$   \\%
$\mathtt{0004h}$ & $\begin{cases}%
                Y = (aX+b)^{g}+c & X \geq d \\%
                Y = (cX+f)       & X < d    \\%
              \end{cases}$ \\%
\bottomrule%
...
\only<2->{%
\begin{tblr}{row{1} = {font=\bfseries}, colspec={cl}, row{2}={bg=green9,fg=black}}%
\toprule%
Encoded value    & Function type                \\%
\midrule%
$\mathtt{0003h}$ & $\begin{cases}%
...
\end{frame}
\begin{frame}{Expressing the TRC}
  For the \emph{DtoB0} case, things are much more complicated...
  ...
  \begin{table}
    \footnotesize
    \caption*{\emph{DtoB0} (excerpt of spec's Table 57)}
...
Encoded value    & Function type                          \\%
\midrule%
$\mathtt{0000h}$ & $Y = (a X + b)^{\gamma} + c$           \\%
$\mathtt{0001h}$ & $Y = a\log_{10}(b X^{\gamma} + c) + d$ \\%
$\mathtt{0002h}$ & $Y = ab^{cX+d}+e$                      \\%
\bottomrule%
\end{tblr}%
}%
\only<2->{%
\begin{tblr}{row{1} = {font=\bfseries}, colspec={cl}, row{2-Z}={bg=red, fg=white}}%
\toprule%
Encoded value    & Function type                          \\%
\midrule%
$\mathtt{0000h}$ & $Y = (a X + b)^{\gamma} + c$           \\%
...
\begin{frame}{Expressing the TRC}
  \begin{itemize}
    \item Both \emph{AtoB0} and \emph{DtoB0} tags support piecewise curves
    \item However, the latter can't represent the second segment of the OETF
          \begin{itemize}
            \item probably an oversight on the ICC part?
          \end{itemize}
    \item This means we must use a sampled curve
    \item We must also apply the offset after/before the conversion matrix
  \end{itemize}
\end{frame}
\begin{frame}{Recap: YCbCr to PCS}
  \begin{itemize}
    ...
    \item For the \emph{DtoB0} version:
          \begin{itemize}
            \item All steps can be expressed individually \emoji{tada}
            \item Sampling the TRCs loses \SI{10}{\percent} of the storage savings
                  \begin{itemize}
                    \item imagine the potential if this were the default... \emoji{pleading-face}
                  \end{itemize}
          \end{itemize}
  \end{itemize}
\end{frame}
...
\begin{frame}{Last step: chromatic adaptation}
  \begin{itemize}
    ...
    \item Alternatively, your CMMs can also insert it \emoji{wink}
  \end{itemize}
\end{frame}
\section{Conclusions}
\begin{frame}{Remarks}
    ...
    \begin{column}{.70\textwidth}<*>
      \begin{itemize}[<*>]
        \item This effort is already live on Krita Next
              \begin{itemize}
                \item \textnormal{\color{red}\emoji{warning} nightly alpha build \emoji{warning}}
              \end{itemize}
